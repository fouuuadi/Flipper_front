...
if (host) {
  ...
  if (import.meta.env.DEV && !new URLSearchParams(location.search).get("session_id")) {
    ...
    const actions: Array<[string, () => void]> = [
      ...
      [
        ...
        () => {
          ...
          setTimeout(
            () => fire({ type: "match:state", status: "playing", sessionId: "dev" }),
            4200,
          );
        },
      ],
      [
        ...
        () => {
          score += 1500;
          fire({ type: "score:update", score, combo: 1 });
        },
      ],
      [
        "Combo x4",
        () => {
          score += 2400;
          fire({ type: "score:update", score, combo: 4 });
        },
      ],
    ...
    console.log(
      "%cDMD sim prêt",
      "color:#ffa502",
      "→ boutons en bas, ou tape dmd.simulate({...}) dans la console",
    );
  }
}
